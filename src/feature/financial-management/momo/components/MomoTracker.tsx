...
export default function MomoTracker() {
        ...
        setTimeout(() => {
            ...
            if (orderId.includes("FAIL")) {
                ...
                setResult({
                    ...
                    momoResponse: {
                        resultCode: 99,
                        message: "Không tìm thấy giao dịch này trên hệ thống Momo."
                    }
                });
            } else {
                setResult({
            ...
            <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm flex flex-col gap-5">
                <div className="flex flex-col gap-2">
                    <h3 className="text-base font-bold text-gray-800 flex items-center gap-2">
                        <Activity size={18} className="text-pink-500" /> Kiểm tra trạng thái Momo Callback
                    </h3>
                    <p className="text-sm text-gray-500">
                        Nhập OrderId của giao dịch để kiểm tra trạng thái thực tế ghi nhận trên hệ thống Momo. (Nhập mã có chứa "FAIL" hoặc "NOTFOUND" để test lỗi).
                    </p>
                </div>

                <div className="flex items-center gap-3">
                    <div className="flex-1 flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus-within:border-pink-500 transition-all">
                        <Search size={18} className="text-gray-400 shrink-0" />
                        <input 
                            type="text" 
                            value={orderId} 
                            onChange={e => setOrderId(e.target.value)} 
                            placeholder="Nhập OrderId (VD: ORD-MOMO-12345)..." 
                    ...
                    <button onClick={handleCheckStatus} disabled={!orderId.trim() || isLoading}
                        className="px-6 py-3 text-sm font-bold text-white bg-pink-500 hover:bg-pink-600 rounded-xl transition-colors shadow-sm disabled:opacity-50 shrink-0 flex items-center gap-2">
                ...
                <div className="bg-gray-900 rounded-3xl p-6 border border-gray-800 shadow-lg flex flex-col gap-4 relative overflow-hidden animate-[fadeIn_0.3s_ease]">
                    ...
                    <div className="bg-black/50 p-4 rounded-xl border border-gray-800 overflow-x-auto">
                        <pre className="text-sm text-green-400 font-mono leading-relaxed">
                            {JSON.stringify(result.momoResponse, null, 4)}
                        </pre>
                    </div>
                </div>
            )}
